# Stock-analysisPurpose
The purpose of this project was to refactor a Microsoft Excel VBA code to collect certain stock information in the year 2017 and 2018 and determine whether or not the stocks are worth investing. This process was originally completed in a similar format, in the class module however, the goal here is to improve the original code with better preformance indicator such as run time.
...
The biggest benefit that occurred as a result of the refactoring was an decrease in macro run time. The original analysis took approximately one second to run, whereas our new analysis took way less time (approximately 0.125 seconds vs 0.8 seconds) to run. I am left to wonder though why each run provides different run time even when on the same year. 

Attached below are the screenshots that indicate the run time from the refactored code.

VBA_Challenge_2017 Screenshot

VBA_Challenge_2018 Screenshot
